...
// --- Componente de Ayuda: Menú de Exportación ---
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        disabled={disabled}
        className="px-4 py-2 text-sm font-semibold text-white bg-gray-700 rounded-md hover:bg-gray-800 disabled:bg-gray-400 flex items-center"
      >
        Exportar
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-56 bg-white rounded-md shadow-lg z-20 ring-1 ring-black ring-opacity-5">
          <div className="py-1">
            <a
              href="#"
              onClick={(e) => { e.preventDefault(); onExport('summary_pdf'); setIsOpen(false); }}
            ...
            <a
              href="#"
              onClick={(e) => { e.preventDefault(); onExport('details_excel'); setIsOpen(false); }}
              className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
            >
              Desglose como Excel (con colores)
            </a>
          </div>
        </div>
      )}
...
export default function MaterialsSummaryClient({ initialSummary, statuses }) {
  ...
  const handleExport = (format) => {
    ...
    if (format === 'summary_pdf') {
      exportSummaryToPdf(summaryData, selectedStatuses);
    }
    if (format === 'details_excel') {
      exportDetailsToExcel(detailedSurgeries, selectedStatuses);
    }
  };
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      <div className="flex flex-col md:flex-row justify-between md:items-start mb-4 pb-4 border-b border-gray-200">
            ...
            {statuses.map(status => (
              <button
                ...
                className={`px-3 py-1 text-sm rounded-full transition-all duration-200 font-medium ${
                  selectedStatusIds.includes(status.id) ? 'ring-2 ring-offset-1 shadow-md' : 'hover:opacity-80'
                }`}
                style={{
                  backgroundColor: selectedStatusIds.includes(status.id) ? status.color : `${status.color}20`,
                  color: selectedStatusIds.includes(status.id) ? 'white' : status.color,
        ...
        <div className="mt-4 md:mt-0">
          <ExportMenu onExport={handleExport} disabled={summaryData.length === 0 && detailedSurgeries.length === 0} />
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {loading ? (
              <tr><td colSpan="3" className="text-center py-10"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600 mx-auto"></div></td></tr>
            ) : summaryData.length > 0 ? (
              summaryData.map(item => (
                <tr key={item.material_id}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.material_name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.material_code}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-indigo-600">{item.total_quantity}</td>
                </tr>
              ))
            ) : (
              <tr><td colSpan="3" className="text-center py-10 text-gray-500">No se encontraron materiales para los filtros seleccionados.</td></tr>
            )}
          </tbody>
        </table>
      </div>
