...
import { UserIcon, BuildingOffice2Icon, BriefcaseIcon, ClipboardDocumentListIcon } from '@heroicons/react/24/outline'

// --- Componente de Autocompletado Reconstruido para una UX Fluida ---
...
function NewMaterialModal({ isOpen, onClose, onSave }) {
  ...
  const handleSave = async () => {
    ...
    if (error) {
      ...
      toast.success('Material creado y añadido al pedido.');
      onSave(data);
      onClose();
    }
  };
    ...
    <div className="fixed inset-0 bg-gray-800 bg-opacity-75 z-50 flex justify-center items-center p-4 transition-opacity">
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md transform transition-all">
        ...
        <div className="space-y-4">
          <input type="text" placeholder="Código (opcional)" value={newMaterial.code} onChange={(e) => setNewMaterial({...newMaterial, code: e.target.value})} className="w-full p-2 border border-gray-300 rounded-md text-gray-900 placeholder:text-gray-500 focus:ring-indigo-500 focus:border-indigo-500" />
          <input type="text" placeholder="* Nombre / Descripción" required value={newMaterial.name} onChange={(e) => setNewMaterial({...newMaterial, name: e.target.value})} className="w-full p-2 border border-gray-300 rounded-md text-gray-900 placeholder:text-gray-500 focus:ring-indigo-500 focus:border-indigo-500" />
          <input type="text" placeholder="Marca / Proveedor" value={newMaterial.brand} onChange={(e) => setNewMaterial({...newMaterial, brand: e.target.value})} className="w-full p-2 border border-gray-300 rounded-md text-gray-900 placeholder:text-gray-500 focus:ring-indigo-500 focus:border-indigo-500" />
          <input type="text" placeholder="Especificación" value={newMaterial.specification} onChange={(e) => setNewMaterial({...newMaterial, specification: e.target.value})} className="w-full p-2 border border-gray-300 rounded-md text-gray-900 placeholder:text-gray-500 focus:ring-indigo-500 focus:border-indigo-500" />
        </div>
        <div className="mt-6 flex justify-end space-x-4">
          ...
          <button onClick={handleSave} disabled={isSaving} className="px-4 py-2 bg-indigo-600 text-white rounded-md disabled:bg-indigo-400 transition-colors">{isSaving ? 'Guardando...' : 'Guardar y Añadir'}</button>
        </div>
      </div>
    </div>
  );
...
function NewSurgeryPageContent() {
  ...
  const [formData, setFormData] = useState({ patient_name: '', doctor_name: '', institution: '', client: '', surgery_date: '', provider: '', initial_note: '' });
  const [isLoading, setIsLoading] = useState(false);
        ...
        <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <form onSubmit={handleSubmit} className="p-4 sm:p-8 bg-white rounded-xl shadow-lg space-y-8">
            ...
            <div className="border-t border-gray-200 pt-8 grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
              ...
              <div><label htmlFor="provider" className="block text-sm font-medium text-gray-700">Proveedor a Solicitar</label><input type="text" name="provider" id="provider" value={formData.provider} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 text-gray-900 shadow-sm placeholder:text-gray-500 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50 transition" placeholder="Ej: Proveedor Principal"/></div>
            ...
            <div className="border-t border-gray-200 pt-8 space-y-6">
              ...
              <div className="relative">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <button type="button" onClick={() => setIsModalOpen(true)} className="px-3 py-1 text-xs font-semibold text-white bg-green-600 rounded-md hover:bg-green-700 transition-colors">
                    + Crear Nuevo
                  </button>
                </div>
